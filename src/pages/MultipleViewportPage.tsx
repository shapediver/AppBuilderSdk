...
export default function ViewPage() {
	...
	const { sessionApi: sessionApi_2 } = useSession(sessionsCreateDto["Bench_2"]);
	const { sessionApi: sessionApi_3 } = useSession(sessionsCreateDto["Bookshelf_1"]);
	const { sessionApi: sessionApi_4 } = useSession(sessionsCreateDto["Bookshelf_2"]);
	...
	const aside = <Tabs defaultValue="bench" style={{ height: "100%"}}>
		...
		<Tabs.Panel value="bookshelf" pt="xs">
			<div style={{ height: "calc(100% - 40px)", overflowY: "auto"}}>
				<ParametersAndExportsAccordionComponent parameters={parameterBookshelfProps} defaultGroupName="Bookshelf parameters" />
			</div>
		</Tabs.Panel>
	</Tabs>;
